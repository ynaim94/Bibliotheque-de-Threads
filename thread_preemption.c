...
#define QUANTUM_MICRO_SEC 10000
...
static void signal_handler_function(int sig_num)//, siginfo_t* info, void *old_context) 
{
  if (sig_num == SIGPROF) {
    /*test*/
    printf("\n[Signal Handler] SIGPROF was raised...");
    // thread_yield();
  }
  else {
...
void init_preemption()
{
  // sets the signal handler for swapping to the scheduler context
    ...
    // scheduling_interuption_handler.sa_handler=signal_handler_function;
  ...
  if (sigaction(SIGPROF, &scheduling_interuption_handler, NULL) == -1) {
    printf("\n[Main Function] An error occurred while initializing the signal handler\n");
    exit(-1);
  }
  ...
  if (setitimer(ITIMER_PROF, &timeslice, NULL) == 0) {
    ...
    printf("An error occurred while executing setitimer...\n");
    exit(-1);
  }
...
int thread_yield(void){
  ...
  if ( SIMPLEQ_EMPTY(&runq) ){
    ...
    /*preemption*/
    /*	if (setitimer(ITIMER_PROF, &timeslice, NULL) == 0) {
	printf(" This is yield \tThe timer was initialized...\n");
	} else {
	printf(" This is yield\t\tAn error occurred while executing setitimer...\n");
	exit(-1);
	}
    */
    /* On effectue le swap */
...
int thread_join(thread_t thread, void **retval){
  ...
  SIMPLEQ_FOREACH(loop, &runq, next){
	...
	// printf("thread n°%d\n", tmp->id);
      ...
      /*preemption*/
      /*	if (setitimer(ITIMER_PROF, &timeslice, NULL) == 0) {
      //	printf(" This is join \tThe timer was initialized...\n");
      } else {
      printf(" This is join\t\tAn error occurred while executing setitimer...\n");
      exit(-1);
      }*/
      /***/
      swapcontext(previous_thread->context, current_thread->context);
...
void thread_exit(void *retval){
  ...
  if(!(SIMPLEQ_EMPTY(&runq))){
    ...
    /*preemption*/
    /*	if (setitimer(ITIMER_PROF, &timeslice, NULL) == 0) {
	printf(" This is exit\tThe timer was initialized...\n");
	} else {
	printf(" This is exit\t\tAn error occurred while executing setitimer...\n");
	exit(-1);
	}*/
    /***/
